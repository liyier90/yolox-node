...
def visualize(bboxes, labels, scores, raw_img):
    img = raw_img
    if bboxes is None:
        return img
    for i, box in enumerate(bboxes):
        ...
        text = "{}:{:.1f}%".format(labels[i], score * 100)
...
if __name__ == "__main__":
    ...
    outputs = yolox_node.run(inputs)

    result_frame = visualize(
        outputs["bboxes"], outputs["bbox_labels"], outputs["bbox_scores"], frame.copy()
    )
